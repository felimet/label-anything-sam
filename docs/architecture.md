...
graph TD
    ...
    SAM3[sam3-ml-backend:9090<br/>SAM3 ML 後端 GPU]
...
SAM3 後端為**選用疊加層**，定義於 `docker-compose.gpu.yml`：

```bash
make up        # 核心服務（不含 SAM3，無需 GPU）
make gpu       # 核心服務 + SAM3（需 NVIDIA GPU）
make gpu-down  # 停止核心 + SAM3
```
