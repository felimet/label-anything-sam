...
| `SAM3_MASK_SELECTION_MODE` | `all` | 推論候選選擇策略：`adaptive`/`top1`/`topk`/`threshold`/`all`。可由 UI `selection_mode` Choices 執行期覆蓋 |
...
| `SAM3_ENABLE_BIDIRECTIONAL_TRACKING` | `true` | 影片後端：追蹤視窗同時向提示畫格的前後各延伸 `MAX_FRAMES_TO_TRACK` 畫格。`false` = 僅前向追蹤 |
