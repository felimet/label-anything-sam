...
- **Prompt sanitization**: all geometric prompts are sanitized to valid normalized `xywh` in `[0,1]`; non-finite or fully out-of-frame prompts are dropped to prevent SAM3 assertion failures.
...
`close_session` is guaranteed via `finally` even on errors. The temporary frame directory is cleaned up automatically when the `with tempfile.TemporaryDirectory()` block exits after `close_session`. VideoRectangle percentages are converted to normalized `xywh` for `add_prompt`, then clamped/sanitized to `[0,1]` with positive area. KeyPoint labels are converted to tiny 2%×2% normalized boxes. Invalid prompts (NaN/Inf or fully out-of-frame) are skipped instead of being forced to border boxes. For text-only requests (no VideoRectangle), metadata is probed from the video file before frame extraction.
...
| `AssertionError` at `boxes_xywh >= 0` / `<= 1` in SAM3 | Upstream prompt payload contains out-of-range or non-finite box coordinates | Backend now sanitizes and drops invalid prompts. If still reproducible, inspect custom task payload for malformed percentages |
