"""WSGI entry point for SAM3 image backend.

Follows the official label-studio-ml-backend example pattern:
  - logging.config.dictConfig for structured logging
  - argparse for CLI mode (label-studio-ml start / direct python invocation)
  - else branch for gunicorn / uWSGI
"""
import os
import argparse
import json
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="SAM3 Image ML Backend")
    ...
    kwargs = _get_kwargs_from_config()
    app = init_app(
